    ...
    for child, x, y, direction in L:
        ...
        match dir: 
            ...
            case 'down-right':
                dx = 5
                dy = 5
        newB = Balloon(x + (dx*i), y + (dy*i), child, direction)
        i *= -1
...
class Balloon:
    ...
    def move(self, app):
        self.checkStatus()
        if (self.x > app.width or 0 > self.y  or self.y > 600):
...
class Tower:
    ...
    def upgrade(self, side, app):
        ...
        if (side == 'left' and ((self.rightPath < 2 or self.leftPath <= 1) or 
              not self.finalUpgrade)):
            type, amount, cost = leftUpgrades[self.name][self.leftPath]
            self.doUpgradeLeft(app, type, amount, cost)
        elif (side == 'right' and ((self.leftPath < 2 or self.rightPath <= 1) or 
              not self.finalUpgrade)): 
        ...
        if self.pierce > 0:
                ...
                for bloon in Balloon.registry:
                    if (distance(bloon.x, bloon.y, self.x, self.y) <= 20 
                        ...
                        # and not (self.bloonToHit == bloon.id and self.hitBloon)
                        # and self.pierce > 0):
                        self.pierce -= 1
